...
export default async function WeekReviewPage({ params }: PageProps) {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      {/* Delivery panel — appears when all pieces are approved */}
      {totalPieces > 0 && approvedCount === totalPieces && (
        <div className="rounded-lg border-2 border-green-200 bg-green-50 p-6">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {/* Metricool CSV */}
            {socialPieces.length > 0 && (
              <MetricoolExportButton weekId={weekId} draft={false} />
            )}

            {/* Direct LinkedIn publish */}
            <Link
              href={`/publish`}
              className="flex items-center gap-2 rounded-lg border border-blue-200 bg-white px-4 py-3 text-sm font-medium text-blue-700 transition-colors hover:bg-blue-50"
            >
            ...
            {/* Draft CSV for preview */}
            <a
              href={`/api/export/metricool?weekId=${weekId}&draft=true`}
              className="flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-4 py-3 text-sm font-medium text-gray-600 transition-colors hover:bg-gray-50"
            >
              <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
                <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
              </svg>
              Download as Draft CSV
            </a>
          </div>
        </div>
      )}
