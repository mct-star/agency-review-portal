-- ============================================================
-- Migration 004 (SAFE / idempotent version)
-- Posting Schedule System: post_types + posting_slots
...
do $$ begin
  if not exists (
    select 1 from pg_policies where tablename = 'post_types' and policyname = 'Anyone can read post types'
  ) then
...
do $$ begin
  if not exists (
    select 1 from pg_policies where tablename = 'post_types' and policyname = 'Admin manages post types'
  ) then
...
create index if not exists idx_posting_slots_company on public.posting_slots(company_id);
...
do $$ begin
  if not exists (
    select 1 from pg_policies where tablename = 'posting_slots' and policyname = 'Admin manages posting slots'
  ) then
...
do $$ begin
  if not exists (
    select 1 from pg_policies where tablename = 'posting_slots' and policyname = 'Client reads own slots'
  ) then
...
-- 3. Seed post types (skip if already seeded)
...
-- 4. Seed AGENCY Bristol posting slots (skip if already seeded)
do $$
declare
  v_company_id uuid;
...
begin
  ...
  -- Check if slots already exist for this company
  select count(*) into v_existing_count from public.posting_slots where company_id = v_company_id;
  if v_existing_count > 0 then
    raise notice 'AGENCY Bristol slots already exist (% found) — skipping', v_existing_count;
    return;
  end if;
